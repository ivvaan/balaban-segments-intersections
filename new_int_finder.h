...
protected:
  ...
  {
    assert(interval_right_index - interval_left_index > 1);
    do {
      ...
      if (interval_left_index + 4 > interval_right_index)//too narrow to recursively divide on two parts 
        goto sis;
      //    else //cut stripe 
      ...
      //else
      if (interval_left_index + 8 > interval_right_index) //too narrow to recursively divide on four parts 
        goto sis;
      // if L contains not so many segments than cut on four parts (works faster for some segment distributions)
      max_call -= 4;
      q = (interval_left_index + m) / 2;
    ...
    while (true) {
      ...
      right_bound = rb += step;
      segments.SetCurStripe(i_f.ENDS[left_bound], i_f.ENDS[right_bound]);
      if (right_bound != left_bound + 1) {
        _FindR(i_f, segments, ladder_start_index, left_bound, right_bound, stack_pos, 0, max_call);
      }
      else
        if (i_f.L_size > 1)
          i_f.SearchInStrip(segments, ladder_start_index);
