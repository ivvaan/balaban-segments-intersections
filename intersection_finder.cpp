...
// InsDel - inserts or deletes segment associated with end_index to/from L
	void CIntersectionFinder::InsDel(int4 end_index,int4 stack_pos,int4 &LS)
	   {
	    int4 j,i;
      ...
      PSeg seg=current_end->s.segment;
	     if (current_end->islast) // if endpoint - delete
	      {
	       i=0; L[LS].segment=seg;
	       while(L[i].segment!=seg)i++;
	       LS--;
	       for (j=i;j<LS;j++)L[j]=L[j+1];
	      }
	     else// if startpoint - insert
	      {
	       for (i=LS-1;(i>-1)&& (!_Under(L[i].segment,seg));i--)
		        L[i+1]=L[i];
		     L[i+1]=current_end->s;
	       LS++;
...
// see InsDel comments
	void CIntersectionFinder::optInsDel(int4 end_index,int4 stack_pos,int4 &LS)
	   {
	    int4 j,i;
	    TSegmEnd *current_end=ENDS+end_index;
      PSeg seg=current_end->s.segment;
...
// in current staircase and place them in Loc.
	int4 CIntersectionFinder::Merge(int4 QB,int4 QE,int4 LE)
	   {
		  int4 LB=0,NL=0;
		  while ((QB<=QE)&&(LB<LE))
...
// Same as Merge but merges only original stairs
	int4 CIntersectionFinder::optMerge(int4 QB,int4 QE,int4 LE)
	   {
		  int4 LB=0,NL=0;
		  while ((QB<=QE)&&(LB<LE))
		    if (_Below(E,L[LB].segment,Q[QB].segment))
	...
	int4 CIntersectionFinder::Split(int4 &step_index,int4 L_size)
	   {
...
// same as Split but  add a part of the parent staircase stairs (inherited stairs) to the new one.
	int4 CIntersectionFinder::optSplit(int4 cur_father_pos,int4 &step_index,int4 L_size)
	   {
		  ...
		  while ((cur_L_pos<L_size)&&(cur_father_pos<=father_last_step))
		     {
		  ...
		  for(;cur_L_pos<L_size;cur_L_pos++)  // father stair case is over. we are adding rest segments the same manner as in suboptimal case
	...
	void CIntersectionFinder::SearchInStrip(int4 QP,int4 lsize)
	   {
		  ...
		  *stack_ptr=NQP=QP;
		  ...
		  while (stack_ptr!=SIS_Stack)
		   {
		    QP=*(--stack_ptr);
		    lsize=Merge(QP+1,NQP,lsize);
		    NQP=QP;
		   }
	   }

// main function to find intersections for fast algorithm
	void CIntersectionFinder::FindR( int4 ladder_start_index,int4 interval_left_index,int4 interval_right_index,
   ...
   ladder_start_index++;
		if (int_numb>LSize)
		  FindR(ladder_cur_index,interval_left_index,interval_right_index,stack_pos,LSize);
		else
		  {
		   m=(interval_left_index+interval_right_index)/2; 
		   FindR(ladder_cur_index,interval_left_index,m,stack_pos,LSize);
		   InsDel(m,stack_pos,LSize);
		   FindR(ladder_cur_index,m,interval_right_index,stack_pos,LSize);
		  }
		 if (ladder_start_index>ladder_cur_index) return;
		 LBoundIdx=interval_left_index; RBoundIdx=interval_right_index;
		 B=ENDS[interval_left_index].x;		 E=ENDS[interval_right_index].x;
...
// main function to find intersections for optimal algorithm
	void CIntersectionFinder::optFindR(int4 father_first_step, int4 ladder_start_index,int4 interval_left_index,int4 interval_right_index,
	 int4 stack_pos,int4 &LSize)
		{
	   int4 m,ladder_cur_index,RSize;
    ...
    ladder_start_index++;     
  	if (int_numb>LSize+1)
		  optFindR(ladder_start_index, ladder_cur_index,interval_left_index,interval_right_index,stack_pos,LSize);
		else
		  {
		   m=(interval_left_index+interval_right_index)/2; 
		   optFindR(ladder_start_index,ladder_cur_index,interval_left_index,m,stack_pos,LSize);
		   optInsDel(m,stack_pos,LSize);
		   optFindR(ladder_start_index,ladder_cur_index,m,interval_right_index,stack_pos,LSize);
		  }
		 if (ladder_start_index>ladder_cur_index) return;
		 LBoundIdx=interval_left_index; RBoundIdx=interval_right_index;
		 B=ENDS[interval_left_index].x;		 E=ENDS[interval_right_index].x;
  ...
  void CIntersectionFinder::balaban_optimal(int4 n,PSeg _Scoll[])
   {
    ...
    optFindR(inherit_each+1,inherit_each,0,  n ,0,LSize);
    optInsDel(n,0,LSize);
    optFindR(inherit_each+1,inherit_each,n,2*n-1,0,LSize);
